...
func IsValid(d Date) bool {
	dayOfMonth := []int{31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31}
	...
	if d.Month == 2 {
		var leap bool = (d.Year%4 == 0 && d.Year%100 != 0 || d.Year%400 == 0)
		if  !leap && d.Day > 28 {
			return false
		}
	}
...
func Calc(date Date) int {
	return date.Year*366*24*60 + date.Month*30*24*60 + date.Day*24*60 + date.Hour*60 + date.Minute
}
